...
\commentlh{The re-organizing looks good to me.}
...
\commentlh{Inputs for a code construct can be viewed at difficult stack layers.
For example, from OS layer, a code construct can take 
inputs through memory read, 
so that accessed memory cells can be used to measure inputs.
From application layer, a code construct can take inputs 
...
\commentlh{since computation cost is most correlated with user-perceived slowdown.} 
...
for a performance slowdown \commentlh{recorded by} multiple 
execution profiles. 
...
\commentlh{because callees' cost is aggregated to their callers, 
while code constructs conducting computation in high complexity 
are buried inside call chains.}\commentty{Why is \texttt{main} ranked at first? 
...
\commentlh{They simply record all direct information to calculate the metrics, 
