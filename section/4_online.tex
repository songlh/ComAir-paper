...
to be instrumented, we propose two optimizations. 
...
\commentlh{
For example, there are four reads on variable \texttt{p} inside 
the loop in Figure~\ref{fig:mysql27287}, two are at 
line 7 (``\texttt{p>=items}'' and ``\texttt{p--}''), 
and the other two are at line 8. 
There is one write on \texttt{p} (``\texttt{p--}'').
Variable \texttt{p} satisfies the requirement of our optimization. 
Since ``\texttt{p>=items}'' is conducted in the loop header 
and it dominates all other accesses,
we only need to instrument the read access inside ``\texttt{p>=items}''.
}

\commentlh{
For the second optimization, if a read or a write is conducted inside a loop and 
the accessed address is not changed across different loop iterations,
we promote the instrumentation to the loop preheader, 
instead of recording the accessed address in every iteration. 
For example, the address of variable \texttt{p} is not changed 
inside the loop in Figure~\ref{fig:mysql27287}.
Therefore, we can record the value of \texttt{p} in the loop's 
...
Instead, \Tool firstly splits 
each monitored BB into two, 
and monitored BBs include BBs inside profiling targets 
and BBs inside functions called from profiling targets.
Then, \Tool labels the event number to be one for 
...
\commentlh{
For example, 
there are in total six basic blocks inside 
the loop in Figure~\ref{fig:mysql27287},
but \Tool only instruments five basic blocks, instead of six. 
As another example, there are 141 basic blocks 
...
whose ids, complexity and ever observed largest costs are listed as follows:
...
Targets 1, 2 and 3 have higher complexity than target 1, 
