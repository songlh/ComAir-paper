...
\textit{\textbf{Read memory size (RMS).}}
...
\textit{\textbf{Data structure size (DSS).}}
The number of distinct accessed elements of a data 
structure could also be used to measure 
input size for a dynamic instance of the code construct~\cite{AlgoProf}. 
...
\textit{\textbf{Library input size (LIS).}}
...
\textit{\textbf{Important integer variable (IIV).}}
...
For example, a recursive implementation to calculate fibonacci numbers shown in Figure~\ref{fig:fib}, 
...
\textit{\textbf{Executed basic blocks (BBs).}}
...
\textit{\textbf{Executed instructions.}}
We can use the executed instruction number of a dynamic 
Instance to represent computation cost for the dynamic instance. 
Similar to executed BBs, we assume each instruction 
roughly takes the same computation cost. 


\textit{\textbf{Loop iterations (LIs).}}
...
\textit{\textbf{Recursive invocations (RIs).}}
As we discussed in Section~\ref{sec:study},
...
\paragraph{\textit{\underline{Inference}}}
...
\paragraph{\textit{\underline{Comparison}}}
...
We use $N$ to represent the set of $n$ sampled number.  
...
\paragraph{\textit{\underline{Input metric}}}
As we discussed in Section~\ref{sec:input},
both RMS+ and DSS have, top-down and bottom-up, 
two methods to analyze profile records for multiple instances of 
...
RMS+ or DSS for every code construct under analysis.
...
To sum up, in in-house setting, we will apply the top-down method, 
while we will use the bottom-up in production-run setting. 
...
we consider cases where a function recursively invoke itself by using integer parameters. 
We do not instrument stack memory \texttt{write}s to save integer 
parameters when a function recursively calls itself. 
Therefore, if the first accesses on these stack memory cells are \texttt{read}s, 
they can increase RMS. 
Since we do not know how the function will change input parameter when recursively calling itself, 
we use the number of active integer parameters, not concrete integer values, as input size.   
...
and we only instrument \texttt{read} conducted on elements in an array or a linked list.
We analyze loops to figure out which address variables holding addresses 
for array element or linked list element. 
If an address variable is read in every iteration of a loop 
and its value is changed by a integer value from its previous value, 
we consider the address variable holds addresses for array elements.
