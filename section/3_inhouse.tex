...
\begin{figure*}
\centering
\subfloat[Inner]{\includegraphics[width=0.22\linewidth]{figure/mysql27287-complexity-n-square}\label{fig:mysql27287-indep}} 
\subfloat[Outer]{\includegraphics[width=0.22\linewidth]{figure/mysql27287-complexity-n-square}\label{fig:mysql27287-outer}}
\subfloat[Bottom-up]{\includegraphics[width=0.22\linewidth]{figure/gcc27733-time-cost-line}\label{fig:mysql27287-merge}} 
\subfloat[Array]{\includegraphics[width=0.22\linewidth]{figure/gcc27733-time-cost-line}\label{fig:mysql27287-inner-array}} \\ 
\vspace{-0.1in}
\caption{XXXX
...
testing or diagnosis~\cite{SongOOPSLA2014,joy.asplos13}.

For all benchmarks used in our experiments,
their bug reports contain at least one bug-triggering inputs. 
For 14 of them, 
the users explicitly explain how to change input sizes to 
observe the described scaling problems. 
We follow the users' 
...
an arithmetic sequence with the input from the bug report as the largest input. 

For each benchmark, we first fit a curve to understand how execution 
time changes with input sizes. 
We then examine fitted cost functions to 
understand whether fitted functions can explain how execution times changes with input sizes. 
For each benchmark, we focus on two fitted functions, one is main function, 
and the other is recursion function. 
As we discussed in Section~\ref{sec:study}, 
the majority of complexity problems are caused by repeated executions 
of a loop or a recursive function. 
We consider the function leads to the repeated executions as recursion function. 
We consider the buggy recursive function or the function containing the outer loop as the recursion 
function for a benchmark.
