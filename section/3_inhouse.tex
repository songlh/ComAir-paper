...
how the whole program execution time scales for all benchmarks, 
as shown by the check-marks 
in Table~\ref{tab:benchmark_info} (the ``main-C'' column).
The minimum similarity value is $0.86$.
...
They can successfully explain how execution time scales for all other benchmarks, 
with minimum similarity value $0.86$ 
(the ``RMS+/Re-C'' column).
...
but the calculated similarity values are less, 
compared with using BBs as cost metric 
(the second value in the ``RMS+/Re-C'' column). 

We have six benchmarks, 
whose buggy code constructs are not array-processing loops or linked-list-processing loops. 
We cannot get input information 
when analyzing their recursion functions using the combination of DSS and LIs 
(``RMS+/Re-C'' column).  
The fitted functions of recursion functions also fail in the two $O(N)$ cases.
For all other cases, the fitted functions of recursion functions can 
explain how execution time scales under the combination of DSS and LIs. 
The combination of DSS and LIs can effectively profile complexity problems caused by 
repeated executions of array-processing loops 
and linked-list-processing loops. 


\textbf{Accuracy Results}
To answer {\bf RQ2},
we manually examine all functions with superlinear complexity reported by \Tool.
They can be divided into two categorizations. 
Some of them are on the call chain between main function to the recursion function.
These functions can also explain how execution time scales with different inputs. 
...
the recursion function from the same benchmark. 
...
Compare with the two combination, DSS plus 
LIs will bring fewer true false positives. 
