...
We use LLVM-5.0.0~\cite{llvm} to instrument programs to be profiled.
...
two methods to profile profile records for multiple instances of 
...
and we will use the bottom-up under production-run setting. 
...
we consider the pointer points to an array element, 
...
its base type needs to be a structure. 
The pointer is also deferenced in every iteration of a loop.
Its value is updated in every iteration, 
and its new value is one field of the structure it is pointing to. 
...
using the \texttt{prev} field of \texttt{node\_t} structure it is pointing to. 
...
To count LIs, for every monitored loop, we instrument a global counter and 
increase the counter value by one in the loop header.
To count RIs, for each monitored recursive function, 
we instrument a global counter and increase the counter 
value in the entry block of the recursive function. 
To count BBs, a naive method is to instrument a global counter
and increase the counter in every basic block. 


{\textit{\textbf{Optimization 4.}}}
